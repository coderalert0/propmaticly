...
FacadeInspectionRule.create(compliance_item: :facade, frequency_in_months: 60, numerical_properties: { number_of_stories: { operator: '>', value: 6 } }, description: 'Facade Inspection Safety Program (FISP), formerly known as Local Law 11, requires periodic inspections of building facades to ensure safety and compliance. This applies to buildings taller than six stories.', )
BedBugInspectionRule.create(compliance_item: :bed_bug, frequency_in_months: 12, numerical_properties: { number_of_residential_units: { operator: '>=', value: 3 } }, description: 'Annual Bed Bug Inspection Report is a requirement to document bed bug infestations and inspection outcomes. Building owners of certain residential properties report on bed bug infestations annually. The report is required for multi-family residential buildings (buildings with three or more units).')
...
# InspectionRule.create(compliance_item: :energy_efficiency_ratings, frequency_in_months: 12, numerical_properties: { square_feet: { operator: '>=', value: 25_000 } }, description: 'Energy Efficiency Ratings Inspection (LL33, LL95) is aimed at promoting energy efficiency and sustainability in buildings. This mandate requires certain buildings to measure their energy performance, receive a rating, and display this information publicly.')
#
...
#
# InspectionRule.create(compliance_item: :annual_property_registration, frequency_in_months: 12, numerical_properties: { }, description: 'The registration helps the NYC Department of Housing Preservation and Development (HPD) maintain an updated database of property ownership and ensures compliance with housing regulations.')
#
# InspectionRule.create(compliance_item: :parking_structure, frequency_in_months: 72, has_properties: { parking_structure: true }, description: 'Parking Structure Inspection Report (LL126) requires owners of parking structures to have their garages inspected every six years, and to file a report with inspection results to the the Department of Buildings')
#
# InspectionRule.create(compliance_item: :gas_line, frequency_in_months: 48, has_properties: { gas_line: true }, description: 'Gas Piping System Inspection Report (LL152) documents the findings from inspections of gas piping systems in buildings, ensuring that they are safe and in compliance with regulations.')
#
# InspectionRule.create(compliance_item: :energy_efficiency_ll_87, frequency_in_months: 120, numerical_properties: { square_feet: { operator: '>=', value: 50_000 } }, description: 'Energy Efficiency Ratings (LL87) involves conducting comprehensive energy audits and retro-commissioning for buildings, documenting findings and recommendations to improve energy efficiency.')

BoilerInspectionRule.create(compliance_item: :low_pressure_boiler, frequency_in_months: 12, has_properties: { low_pressure_boiler: true }, description: 'Low Pressure Boiler Inspection focuses on the overall condition and safety of the boiler, including checks for leaks, proper functioning of controls, and the integrity of safety devices.')

BoilerInspectionRule.create(compliance_item: :high_pressure_boiler, frequency_in_months: 12, has_properties: { high_pressure_boiler: true }, description: 'High Pressure Boilers must have two inspections – an internal inspection and an external inspection. These inspections must be conducted within the same cycle but should be performed approximately six months apart.')

# InspectionRule.create(compliance_item: :energy_benchmarking, frequency_in_months: 12, numerical_properties: { square_feet: { operator: '>=', value: 50_000 } }, description: 'Energy Benchmarking Report (LL84) requires large buildings to annually benchmark their energy and water usage.')
#
# InspectionRule.create(compliance_item: :energy_efficiency_ll_133, frequency_in_months: 120, numerical_properties: { square_feet: { operator: '>=', value: 50_000 } }, description: 'Energy Efficiency Report (LL133) mandates buildings undergo comprehensive energy audits and retro-commissioning every ten years to identify efficiency improvements')
#
